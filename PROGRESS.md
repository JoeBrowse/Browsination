...
- **Editing and backdating** run through one registry: every log type a module writes declares a `LogTypeDef`, and History (app tray, and a link at the top of Insights) lists everything ever logged, newest first, with module and type filters. Tapping a row opens the generic editor, or the module's own sheet where a raw edit would not add up. Adding from History covers the self-contained types; the rest are backdated from where they are logged (the drink and caffeine sheets, the break and routine forms, the practice sheet, the check-in day switcher, the habit fill-in strip, a done item's "Done on").
